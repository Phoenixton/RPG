...
public abstract class Creature extends Entity {
	...
	public void moveX(){
		if(xMove > 0){ //moving right
			int tx = (int) (x + xMove + bounds.x + bounds.width) / Tile.TILEWIDTH; //gives the x coordinate of tile we are trying to move into
			...
			int tx = (int) (x + xMove + bounds.x) / Tile.TILEWIDTH; //gives the x coordinate of tile we are trying to move into
			if(!collisionWithTile(tx, (int) (y + bounds.y) / Tile.TILEHEIGHT) && !collisionWithTile(tx, (int)(y + bounds.y + bounds.height) / Tile.TILEHEIGHT)){ //first checking upper right corner then lower right corner of the box. if it's not solid, then we are good to move
				x += xMove;
			}else{
